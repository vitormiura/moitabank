import Logo from "../public/logo.png";
import Image from "next/image";
import Head from "next/head";

export default function Footer() {
  return (
    <>
      <Head>
        <title>Moitabank</title>
        <meta name="description" content="Generated by create next app" />
        <link rel="logo" href="/favicon.ico" />
      </Head>
      <footer className="p-4 rounded-lg shadow md:px-6 md:py-8 bg-stone-200">
        <div className="sm:flex sm:items-center sm:justify-between max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <a href="" className="flex items-center mb-4 sm:mb-0">
            <Image src={Logo} className="mr-3 h-8" alt="Moitabank Logo"></Image>
            <span className="self-center text-2xl font-semibold whitespace-nowrap text-black">
              Moitabank
            </span>
          </a>
          <ul className="flex flex-wrap items-center mb-6 text-sm sm:mb-0 text-gray-500">
            <li>
              <a href="#" className="mr-4 hover:underline md:mr-6 ">
                About
              </a>
            </li>
            <li>
              <a href="#" className="mr-4 hover:underline md:mr-6">
                Privacy Policy
              </a>
            </li>
            <li>
              <a href="#" className="mr-4 hover:underline md:mr-6 ">
                Licensing
              </a>
            </li>
            <li>
              <a href="#" className="hover:underline">
                Contact
              </a>
            </li>
          </ul>
        </div>
        <hr className="my-6 sm:mx-auto border-gray-700 lg:my-8"></hr>
        <span className="block text-sm text-black-400 sm:text-center">
          © 2022{" "}
          <a href="https://github.com/vitormiura" className="hover:underline">
            Vitor Miura
          </a>
          . All Rights Reserved.
        </span>
      </footer>
    </>
  );
}
